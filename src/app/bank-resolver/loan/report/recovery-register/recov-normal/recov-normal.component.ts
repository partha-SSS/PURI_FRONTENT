...
export class RecovNormalComponent implements OnInit {
  ...
  exportToExcel() {
    ...
    this.reportData.forEach((account) => {
      account.blockwiserecovery.forEach((block) => {
        block.gmloantrans.forEach((loan) => {
          flatData.push({
            acc_cd: account.acctype.acc_cd,
            acc_name: account.acctype.acc_name,
            block_name: block.blocktype.block_name,
            loan_id: loan.loan_id,
            cust_name: loan.cust_name,
            trans_dt: loan.trans_dt,
            adv_prn_recov:loan.adv_prn_recov,
            curr_prn_recov: loan.curr_prn_recov,
            ovd_prn_recov: loan.ovd_prn_recov,
            curr_intt_recov: loan.curr_intt_recov,
            ovd_intt_recov: loan.ovd_intt_recov,
            penal_intt_recov: loan.penal_intt_recov,
            recov_amt: loan.recov_amt,
            last_intt_calc_dt:loan.last_intt_calc_dt
          });
        });
      });
    });
    ...
    const workbook: XLSX.WorkBook = { Sheets: { 'Loans Data': worksheet }, SheetNames: ['Loans Data'] };
    const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });

    // Save as Excel file
    const data: Blob = new Blob([excelBuffer], { type: 'application/octet-stream' });
    saveAs(data, 'loan_recovery.xlsx');
  }
